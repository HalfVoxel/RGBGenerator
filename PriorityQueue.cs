...
public class BinaryHeap<T> { 
	...
	public BinaryHeap ( int numberOfElements ) { 
		binaryHeap = new Pixel[numberOfElements]; 
		numberOfItems = 2;
	} 
	...
	public Pixel GetNode (int i) {
		return binaryHeap[i];
	}

	/** Adds a node to the heap */
	public void Add(Pixel node, T reference) {
		...
		if (numberOfItems == binaryHeap.Length) {
			...
			if (newSize > 1<<18) {
				throw new System.Exception ("Binary Heap Size really large (2^18). A heap size this large is probably the cause of running in an infinite loop. " +
				                            "\nRemove this check (in BinaryHeap.cs) if you are sure that it is not caused by a bug");
			}

			Pixel[] tmp = new Pixel[newSize];
			T[] tmp2 = new T[newSize];
		...
		refHeap [numberOfItems] = reference;
		//node.heapIndex = numberOfItems;//Heap index

		int bubbleIndex = numberOfItems;
		uint nodeF = node.QueueScore;

		while (bubbleIndex != 1) {
			int parentIndex = bubbleIndex / 2;

			if (nodeF < binaryHeap[parentIndex].QueueScore) {
				...
				refHeap [bubbleIndex] = refHeap [parentIndex];
				refHeap [parentIndex] = reference;
	...
	public Pixel Remove() {
		...
		do {
			...
			if (p2 + 1 <= numberOfItems) {
				// Both children exist
				if (binaryHeap[parent].QueueScore >= binaryHeap[p2].QueueScore) {
					swapItem = p2;//2 * parent;
				}
				if (binaryHeap[swapItem].QueueScore >= binaryHeap[p2 + 1].QueueScore) {
					swapItem = p2 + 1;
				}
			...
			if (parent != swapItem) {
				Pixel tmpIndex = binaryHeap[parent];
				T tmpIndex2 = refHeap[parent];
				...
				refHeap[parent] = refHeap[swapItem];
				refHeap[swapItem] = tmpIndex2;
	...
	public void Rebuild () {
		...
		for (int i=2;i<numberOfItems;i++) {
			int bubbleIndex = i;
			Pixel node = binaryHeap[i];
			T reference = refHeap [i];
			uint nodeF = node.QueueScore;
			while (bubbleIndex != 1) {
				int parentIndex = bubbleIndex / 2;
